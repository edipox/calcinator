...
### Bug Fixes
* [#34](https://github.com/C-S-D/calcinator/pull/34) - [@jeffutter](https://github.com/jeffutter)
  * `Calcinator.Resources.Ecto.Repo.update/4` was calling `module.update/2` with the return of `module.changeset/2`, `module.changeset/2` returns `{:ok, changeset}` but `module.update/2` expects only `changeset`. `Calcinator.Resources.Ecto.Repo.update/4` now unwraps the call to `module.changeset/2` inside a `with` to pass along any error responses.
